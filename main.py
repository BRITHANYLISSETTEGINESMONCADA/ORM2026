...
def _generar_xml(factura: FacturaRequest, clave_acceso: str, numero_factura: str) -> str:
    detalles_xml = ''.join(
        f'''
        <detalle>
            <descripcion>{xml_escape(p.nombre)}</descripcion>
...
<factura id="comprobante" version="1.1.0">
    ...
    <infoFactura>
        ...
        <razonSocialComprador>{xml_escape(factura.cliente.nombre)}</razonSocialComprador>
        <identificacionComprador>{xml_escape(factura.cliente.identificacion)}</identificacionComprador>
...
def _enviar_correo_factura(factura: FacturaRequest, clave_acceso: str, numero_factura: str, xml: str) -> None:
    if not EMAIL_HOST:
        logger.info(
            ...
            f'(factura {numero_factura}, clave {clave_acceso})'
        )
        return
